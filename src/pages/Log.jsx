// src/pages/Log.jsx — v10.4: Horizontal pill bar, AnimatePresence form panel, SVG icons
...
import {
  IconBrain, IconCamera, IconCheck, IconX,
  IconUtensils, IconDumbbell, IconDrop, IconPill,
  IconMoon, IconScale, IconSmile, IconAlertCircle, IconZap, IconHeart,
} from "../components/Icons.jsx";

export default function Log({ data, setData, initialForm, onDelete }) {
  const { theme: G } = useTheme();
  const [active, setActive] = useState(initialForm || null);
  const today = td();

  const [aiText, setAiText] = useState("");
  const [aiTextL, setAiTextL] = useState(false);
  const [aiTextItems, setAiTextItems] = useState([]);
  const photoRef = useRef();
  const [photoL, setPhotoL] = useState(false);
  ...
  const [healthL, setHealthL] = useState(false);
  const [healthR, setHealthR] = useState(null);
  const healthFileRef = useRef();
  ...
  const analyzeText = async () => {
    ...
    setAiTextL(true); setAiTextItems([]);
    try {
      const txt = await callClaude({
        system: `Nutrition analyst. Accurate macros. Pure JSON array. ${data.profile.allergies ? `Flag any items containing: ${data.profile.allergies}.` : ""}
Return ONLY valid JSON array: [{"food":"item with portion","calories":N,"protein":N,"carbs":N,"fat":N}]`,
    ...
    } catch (e) { setAiTextItems([{ error: e.message || "Failed." }]); }
    setAiTextL(false);
  };

  const snapPhoto = async (file) => {
    setPhotoL(true); setAiTextItems([]);
    try {
      const b64 = await toB64(file); const bd = b64.split(",")[1];
  ...
  const analyzeHealth = async (file) => {
    setHealthL(true); setHealthR(null);
    try {
      const b64 = await toB64(file); const bd = b64.split(",")[1];
      const txt = await callClaude({ system: `Extract health data from Apple Health screenshots. JSON only:\n{"type":"sleep|heart_rate|ecg|blood_oxygen|respiratory|steps|vo2max|workout|other","data":{...},"date":"YYYY-MM-DD","summary":"brief"}`, messages: [{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: file.type || "image/jpeg", data: bd } }, { type: "text", text: "Extract health data." }] }] });
      ...
      nd.healthImports = [...nd.healthImports, { id: uid(), date: today, type: parsed.type, summary: parsed.summary }];
      setData(nd); sv(nd); haptic();
    } catch (e) { setHealthR({ error: e.message || "Failed." }); }
    setHealthL(false);
  };

  // ── Category definitions ──────────────────────────────────────────────────
  const cats = [
    { id: "meal",     label: "Meal",     Icon: IconUtensils,    color: G.moss },
    { id: "exercise", label: "Exercise", Icon: IconDumbbell,    color: G.orange },
    { id: "water",    label: "Water",    Icon: IconDrop,        color: G.teal },
    { id: "supps",    label: "Supps",    Icon: IconPill,        color: G.purple },
    { id: "sleep",    label: "Sleep",    Icon: IconMoon,        color: G.indigo },
    { id: "body",     label: "Body",     Icon: IconScale,       color: G.blue },
    { id: "mood",     label: "Mood",     Icon: IconSmile,       color: G.pink },
    { id: "pain",     label: "Pain",     Icon: IconAlertCircle, color: G.red },
    { id: "postWO",   label: "Post-WO",  Icon: IconZap,         color: G.amber },
    { id: "health",   label: "Health",   Icon: IconHeart,       color: G.red },
  ];
  ...
  // ── Today feed ───────────────────────────────────────────────────────────
  const todayFeed = [];
  data.nutrition.filter(n => n.date === today).forEach(n => todayFeed.push({ k: n.id, arr: "nutrition", label: n.food, sub: `${n.meal} · ${n.calories || 0} cal · P:${n.protein || 0}g`, color: G.moss }));
  data.training.filter(t => t.date === today).forEach(t => todayFeed.push({ k: t.id, arr: "training", label: t.name, sub: t.type === "Strength" ? `${t.sets || ""}×${t.reps || ""} @ ${t.weight || ""}lbs` : `${t.type} · ${t.duration || ""}min`, color: G.orange }));
  data.hydration.filter(h => h.date === today).forEach(h => todayFeed.push({ k: h.id, arr: "hydration", label: `${h.oz}oz ${h.type || "Water"}`, sub: h.time || "", color: G.teal }));
  data.supplements.filter(s => s.date === today).forEach(s => todayFeed.push({ k: s.id, arr: "supplements", label: s.name, sub: `${s.timing}${s.dosage ? ` · ${s.dosage}` : ""}`, color: G.purple }));
  data.sleep.filter(s => s.date === today).forEach(s => todayFeed.push({ k: s.id, arr: "sleep", label: `${s.hours}hrs sleep`, sub: s.quality, color: G.indigo }));
  data.lifestyle.filter(l => l.date === today).forEach(l => todayFeed.push({ k: l.id, arr: "lifestyle", label: `Energy ${l.energy}/10 · Stress ${l.stress}/10`, sub: l.mood, color: G.pink }));
  data.bodyMetrics.filter(b => b.date === today).forEach(b => todayFeed.push({ k: b.id, arr: "bodyMetrics", label: `${b.weight || "?"}lbs${b.bodyFat ? ` · ${b.bodyFat}% BF` : ""}`, sub: "Body", color: G.blue }));
  data.painLog.filter(p => p.date === today).forEach(p => todayFeed.push({ k: p.id, arr: "painLog", label: `${p.location} pain`, sub: `${p.type} · ${p.severity}/10`, color: G.red }));
  data.postWorkout.filter(p => p.date === today).forEach(p => todayFeed.push({ k: p.id, arr: "postWorkout", label: `Post-WO: RPE ${p.rpe}/10`, sub: `${p.mood} · Energy ${p.energy}/10`, color: G.amber }));

  // ── Active category color ────────────────────────────────────────────────
  const activeCat = cats.find(c => c.id === active);
  const accentColor = activeCat?.color || G.moss;
  ...
  return (
    <div>
      ...
      {/* ── AI Entry Panel ───────────────────────────────────────────────── */}
      <Glass
        glow={`radial-gradient(circle at 25% 50%, ${G.moss}1a, transparent 65%)`}
        style={{ marginBottom: 16, borderRadius: 20, padding: 18 }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
          <IconBrain size={15} color={G.moss} />
          <span style={{ fontSize: 13, fontWeight: 700, color: G.moss, letterSpacing: 0.2 }}>Log Anything</span>
        </div>
        <div style={{ fontSize: 11, color: G.dim, marginBottom: 10, lineHeight: 1.5 }}>
          Food, workouts, sleep, supplements — AI parses the macros and logs it.
        </div>
        <textarea
          value={aiText}
          onChange={e => setAiText(e.target.value)}
          placeholder="e.g. 150g chicken breast, cup of brown rice, and 16oz water"
          style={{
            width: "100%", background: G.glass2, border: `1px solid ${G.glassBorder}`,
            borderRadius: 12, padding: "10px 13px", color: G.txt, fontSize: 14,
            outline: "none", fontFamily: "inherit", boxSizing: "border-box",
            minHeight: 52, resize: "none", lineHeight: 1.5,
          }}
        />
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <Btn
            onClick={analyzeText}
            disabled={aiTextL || !aiText.trim()}
            sx={{ flex: 1, padding: 11, display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}
            v={aiText.trim() ? "primary" : "secondary"}
          >
            <IconBrain size={14} color="currentColor" />
          ...
          <button
            ...
            style={{
              background: G.glass2, border: `1px solid ${G.glassBorder2}`,
              borderRadius: 14, padding: "11px 15px", color: G.sub,
              cursor: "pointer", fontFamily: "inherit", opacity: photoL ? 0.5 : 1,
              display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
              fontSize: 13, fontWeight: 600,
            }}
          >
            <IconCamera size={16} color={G.sub} />
            {photoL ? "…" : "Photo"}
          </button>
        ...
        <AnimatePresence>
          {aiTextItems.length > 0 && !aiTextItems[0]?.error && (
            <motion.div
              ...
              <div style={{ marginTop: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <IconCheck size={13} color={G.moss} />
                    <span style={{ fontSize: 12, fontWeight: 700, color: G.moss }}>{aiTextItems.length} item{aiTextItems.length > 1 ? "s" : ""} parsed</span>
                  </div>
                  <Btn onClick={addAllAiItems} v="primary" sx={{ fontSize: 11, padding: "5px 12px" }}>Log All</Btn>
                </div>
                {aiTextItems.map((item, i) => (
                  <div key={i} style={{
                    background: G.glass, borderRadius: 12, padding: "9px 12px", marginBottom: 4,
                    border: `1px solid ${G.glassBorder}`, display: "flex", alignItems: "center", gap: 10,
                  }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, color: G.txt, fontWeight: 500 }}>{item.food}</div>
                      <div style={{ fontSize: 11, color: G.dim }}>P:{item.protein}g · C:{item.carbs}g · F:{item.fat}g</div>
                    </div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: G.moss, minWidth: 36, textAlign: "right" }}>{item.calories}</div>
                    <button onClick={() => addAiItem(item)} style={{
                      background: `${G.moss}20`, border: `1px solid ${G.moss}30`, borderRadius: 8,
                      width: 28, height: 28, color: G.moss, cursor: "pointer", fontSize: 16,
      ...
      {stacks.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: G.dim, letterSpacing: 1.2, marginBottom: 8 }}>SUPPLEMENT STACKS</div>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {stacks.map(stack => (
              <button key={stack.id} onClick={() => logStack(stack)} style={{
                background: `${G.purple}15`, border: `1px solid ${G.purple}25`,
                borderRadius: 20, padding: "7px 14px", color: G.purple,
                fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit",
                display: "flex", alignItems: "center", gap: 6,
              }}>
                <IconPill size={13} color={G.purple} />
                {stack.name}
              </button>
            ))}
          </div>
      ...
      {/* ── Category Pill Bar ────────────────────────────────────────────── */}
      <div style={{
        display: "flex", gap: 7, overflowX: "scroll",
        paddingBottom: 6, paddingLeft: 20, paddingRight: 20,
        marginLeft: -20, marginRight: -20,
        scrollbarWidth: "none", msOverflowStyle: "none",
        WebkitOverflowScrolling: "touch", marginBottom: 4,
      }}>
        {cats.map((cat, idx) => {
          ...
          return (
            <motion.button
              ...
              style={{
                ...
                background: isActive ? `${cat.color}22` : G.glass,
                border: `1px solid ${isActive ? cat.color + "55" : G.glassBorder}`,
                cursor: "pointer", fontFamily: "inherit",
                transition: "background .15s, border-color .15s",
                boxShadow: isActive ? `0 0 12px ${cat.color}22` : "none",
              }}
            >
              <cat.Icon size={14} color={isActive ? cat.color : G.dim} />
              <span style={{
                fontSize: 12, fontWeight: 700, color: isActive ? cat.color : G.sub,
                whiteSpace: "nowrap",
              }}>{cat.label}</span>
            </motion.button>
          );
            ...
            <div style={{
              ...
              {active === "meal" && <>
                {recentMeals.length > 0 && (
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 10, fontWeight: 700, color: G.dim, letterSpacing: 1, marginBottom: 7 }}>REPEAT RECENT</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                      {recentMeals.map((m, i) => (
                        <button key={i} onClick={() => repeatMeal(m)} style={{
                          background: G.glass2, border: `1px solid ${G.glassBorder}`,
                          borderRadius: 10, padding: "4px 11px", color: G.sub,
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ flex: 1 }}><Fld label="Calories" type="number" value={mealF.calories} set={v => setMealF({ ...mealF, calories: v })} /></div>
                  <div style={{ flex: 1 }}><Fld label="Protein" type="number" value={mealF.protein} set={v => setMealF({ ...mealF, protein: v })} /></div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <div style={{ flex: 1 }}><Fld label="Carbs" type="number" value={mealF.carbs} set={v => setMealF({ ...mealF, carbs: v })} /></div>
              ...
              {active === "exercise" && <>
                ...
                <div style={{ position: "relative", marginBottom: 14 }}>
                  ...
                  {exSuggestions.length > 0 && (
                    <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: G.bg === "#f4f3ef" ? "#f0efe9" : "#000000", border: `1px solid ${G.glassBorder2}`, borderRadius: 12, zIndex: 50, overflow: "hidden" }}>
                      {exSuggestions.map((s, i) => (
                        <button key={i} onClick={() => { setExF({ ...exF, name: s }); setExSearch(""); }}
                          style={{ width: "100%", padding: "10px 14px", background: "transparent", border: "none", color: G.sub, fontSize: 13, cursor: "pointer", fontFamily: "inherit", textAlign: "left", borderBottom: i < exSuggestions.length - 1 ? `1px solid ${G.glassBorder}` : "none" }}>
                          {s}
                        </button>
                      ))}
                    </div>
                ...
                {(exF.type === "Cardio" || exF.type === "Sport") && (
                  <div style={{ display: "flex", gap: 8 }}>
                    <div style={{ flex: 1 }}><Fld label="Duration (min)" type="number" value={exF.duration} set={v => setExF({ ...exF, duration: v })} /></div>
                    <div style={{ flex: 1 }}><Fld label="Distance (mi)" type="number" value={exF.distance} set={v => setExF({ ...exF, distance: v })} /></div>
                  </div>
                )}
              ...
              {active === "water" && <>
                <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
                  {[8, 12, 16, 20, 24, 32].map(oz => (
                    <button key={oz} onClick={() => { quickWater(oz); setActive(null); }} style={{
                      background: `${G.teal}15`, border: `1px solid ${G.teal}25`,
                      borderRadius: 20, padding: "8px 14px", color: G.teal,
              ...
              {active === "body" && <>
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <div style={{ flex: 1 }}><Fld label="Body Fat %" type="number" value={bodyF.bodyFat} set={v => setBodyF({ ...bodyF, bodyFat: v })} step=".1" /></div>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
              ...
              {active === "health" && <>
                <div style={{ textAlign: "center", padding: "4px 0 14px" }}>
                  <div style={{ fontSize: 12, color: G.sub, lineHeight: 1.6, marginBottom: 14 }}>
                    Screenshot any Apple Health screen — AI extracts the data automatically.
                  </div>
                  <Btn
                    onClick={() => healthFileRef.current?.click()}
                    disabled={healthL}
                    sx={{ padding: "12px 24px", fontSize: 14, display: "inline-flex", alignItems: "center", gap: 8 }}
                  >
                    <IconCamera size={15} color="currentColor" />
                    {healthL ? "Analyzing…" : "Upload Screenshot"}
                  </Btn>
                ...
                {healthR && !healthR.error && (
                  <div style={{ background: G.glass, borderRadius: 12, padding: 12, marginTop: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                      <IconCheck size={13} color={G.moss} />
                      <span style={{ fontSize: 12, fontWeight: 700, color: G.moss }}>Imported: {healthR.type?.replace(/_/g, " ")}</span>
                    </div>
      ...
      {todayFeed.length > 0 && (
        <div style={{ marginTop: 22 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: G.dim, letterSpacing: 1.2, marginBottom: 10 }}>TODAY'S LOG</div>
          {todayFeed.map((f, i) => (
            <EI
              key={f.k + i}
              primary={f.label}
              secondary={f.sub}
              color={f.color}
              onDelete={onDelete ? () => onDelete(f.arr, f.k) : undefined}
            />
          ))}
        </div>
      )}
